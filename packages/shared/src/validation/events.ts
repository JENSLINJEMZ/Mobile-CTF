...
export const eventStatusSchema = z.enum(['DRAFT', 'SCHEDULED', 'RUNNING', 'ENDED']);
export const unlockRuleTypeSchema = z.enum(['TIME', 'PREREQUISITE', 'SCORE', 'ALWAYS']);
...
const slugSchema = z
  ...
  .trim()
  .toLowerCase()
  .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, 'URL-safe slug, e.g. capture-the-flag-2026')
  .max(120);
...
function validateTimeWindow(event: { startsAt?: string; endsAt?: string }, ctx: z.RefinementCtx) {
...
export const createEventSchema = baseEventSchema.superRefine(validateTimeWindow);

export const updateEventSchema = baseEventSchema.partial().superRefine(validateTimeWindow);
