...
export function decodeJwt(token: string): DecodedJwt {
  ...
  if (parts.length !== 3) {
    errors.push('Token must have three dot-separated segments (header.payload.signature)');
    return {
      header: null,
      payload: null,
      signature: '',
  ...
  try {
    ...
    header = typeof parsed === 'object' && parsed !== null ? (parsed as Record<string, unknown>) : null;
  } catch {
  ...
  if (payload) {
    ...
    if (typeof exp === 'number') {
      ...
      if (exp <= now) errors.push(`Token expired at ${new Date(exp * 1000).toISOString()}`);
    }
