...
function makeToken(payload: Record<string, unknown>, header: Record<string, unknown> = { alg: 'HS256', typ: 'JWT' }): string {
...
describe('decodeJwt', () => {
  it('decodes a well-formed token', () => {
    ...
    const token = makeToken({ sub: 'user-1', name: 'Alice', iat: now, exp: now + 3600 });
  ...
  it('flags an expired token', () => {
    ...
    const result = decodeJwt(token);
    expect(result.errors.some((error) => error.startsWith('Token expired'))).toBe(true);
