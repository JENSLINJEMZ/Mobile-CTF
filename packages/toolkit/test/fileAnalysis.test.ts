...
describe("sniffFileType", () => {
  it("detects JPEG, PNG, GIF, GIF89a and PDF", () => {
    expect(sniffFileType(Uint8Array.from([0xff, 0xd8, 0xff, 0xe0]))?.name).toBe(
      "JPEG image",
    );
    expect(
      sniffFileType(
  ...
  it("detects zip, gzip and sqlite", () => {
    expect(
      sniffFileType(Uint8Array.from([0x50, 0x4b, 0x03, 0x04, 0x00, 0x00]))
        ?.name,
    ).toBe("ZIP archive");
    expect(sniffFileType(Uint8Array.from([0x1f, 0x8b, 0x08]))?.name).toBe(
      "GZIP archive",
    );
    expect(sniffFileType(utf8ToBytes("SQLite format 3\x00"))?.name).toBe(
      "SQLite database",
    );
  });
...
describe("parseExif (JPEG branch)", () => {
  ...
  it("returns null for a JPEG without EXIF", () => {
    const plain = Uint8Array.from([
      0xff, 0xd8, 0xff, 0xe0, 0x00, 0x04, 0xff, 0xd9,
    ]);
