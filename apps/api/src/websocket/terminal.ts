...
export function attachTerminalNamespace(io: Server): void {
  ...
  terminalNamespace.on('connection', (socket) => {
    ...
    socket.on('terminal:join', (payload: unknown, ack?: (reply: JoinAck) => void) => {
      void joinHandler(socket, payload, ack);
    });

    socket.on('terminal:input', (payload: unknown, ack?: (reply: InputAck) => void) => {
  ...
  terminalEvents.on('output', (event: TerminalOutputEvent) => {
    terminalNamespace.to(roomFor(event.sessionId)).emit('terminal:output', event);
  });
...
function parseInput(payload: unknown): { sessionId: string | null; data: string | undefined } {
  const sessionId = parseSessionId(payload);
  const data =
