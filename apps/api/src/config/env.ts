...
export const env = {
  ...
  jwtRefreshSecret:
    process.env.JWT_REFRESH_SECRET ?? "dev-insecure-refresh-secret",
  accessTtlSeconds: Number(
    process.env.JWT_ACCESS_TTL_SECONDS ?? AUTH.ACCESS_TTL_SECONDS,
  ),
  refreshTtlSeconds: Number(
    process.env.JWT_REFRESH_TTL_SECONDS ?? AUTH.REFRESH_TTL_SECONDS,
  ),
  ...
  rateLimitLogin: Number(
    process.env.RATE_LIMIT_LOGIN ?? RATE_LIMITS.LOGIN_PER_MIN,
  ),
  rateLimitGeneral: Number(
    process.env.RATE_LIMIT_GENERAL ?? RATE_LIMITS.GENERAL_PER_MIN_PER_USER,
  ),
  rateLimitSubmission: Number(
    process.env.RATE_LIMIT_SUBMISSION ??
      RATE_LIMITS.SUBMISSION_PER_MIN_PER_USER,
  ),
  ...
  sandboxDockerHost: process.env.SANDBOX_DOCKER_HOST ?? null,
  sandboxDockerTlsCa: process.env.SANDBOX_DOCKER_TLS_CA ?? null,
  sandboxDockerTlsCert: process.env.SANDBOX_DOCKER_TLS_CERT ?? null,
  sandboxDockerTlsKey: process.env.SANDBOX_DOCKER_TLS_KEY ?? null,
