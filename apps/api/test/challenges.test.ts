...
async function registerAndGetToken(): Promise<string> {
  ...
  return (res.body.data as { tokens: { accessToken: string } }).tokens
    .accessToken;
}
...
beforeAll(async () => {
  const category = await prisma.challengeCategory.create({
    data: {
      name: `Test Crypto ${runSuffix}`,
      slug: `test-crypto-${runSuffix.slice(-8)}`,
      icon: "lock",
      sortOrder: 1,
    },
  });
  ...
  const tag = await prisma.tag.create({
    data: {
      name: `TestTag ${runSuffix}`,
      slug: `test-tag-${runSuffix.slice(-8)}`,
    },
  });
  ...
  adminToken = (adminLogin.body.data as { tokens: { accessToken: string } })
    .tokens.accessToken;
  ...
  const hintB2 = await api
    ...
    .set("Authorization", `Bearer ${adminToken}`)
    .send({
      title: "Expensive hint",
      body: "The flag is two words.",
      penaltyPoints: 50,
      sortOrder: 1,
    })
    .expect(201);
...
describe("GET /api/challenges (browse)", () => {
  it("lists only published challenges for anonymous users", async () => {
    ...
    const body = res.body as {
      data: { items: ChallengeSummaryDto[]; meta: { total: number } };
    };
  ...
  it("filters by difficulty and honours pagination", async () => {
    ...
    expect(res.status).toBe(200);
    const body = res.body as {
      data: { items: ChallengeSummaryDto[]; meta: { totalPages: number } };
    };
...
describe("POST /api/challenges/:id/submissions", () => {
  ...
  it("deducts unlocked hint penalties", async () => {
    ...
    const detail = await api
      ...
      .set("Authorization", `Bearer ${userAToken}`);
    const hint = (detail.body.data as ChallengeDetailDto).hints.find(
      (h) => h.id === hintBId,
    );
...
describe("POST /api/challenges/:id/hints/:hintId/unlock", () => {
  it("reveals hint body to the unlocking user only after unlock", async () => {
    ...
    const locked = (before.body.data as ChallengeDetailDto).hints.find(
      (h) => h.id === hintB2Id,
    );
    ...
    const after = await api
      .get(`/api/challenges/${challengeBId}`)
      .set("Authorization", `Bearer ${userCToken}`);
    const unlocked = (after.body.data as ChallengeDetailDto).hints.find(
      (h) => h.id === hintB2Id,
    );
  ...
  it("requires authentication", async () => {
    const res = await api.post(
      `/api/challenges/${challengeBId}/hints/${hintBId}/unlock`,
    );
    expect(res.status).toBe(401);
  });
});
...
describe("admin challenge CRUD", () => {
  ...
  it("updates a challenge and snapshots a new version", async () => {
    ...
    const versions = await prisma.challengeVersion.count({
      where: { challengeId: challengeAId },
    });
