...
function once(socket: Socket, event: string, timeoutMs: number): Promise<unknown> {
...
describe('GET /api/leaderboard', () => {
  ...
  it('contains the solver ranked by score after solving', async () => {
    ...
    for (let i = 1; i < data.entries.length; i += 1) {
      expect(data.entries[i - 1]?.score).toBeGreaterThanOrEqual(data.entries[i]?.score ?? 0);
    }
    expect(data.entries.every((e) => e.rank === data.entries.indexOf(e) + 1)).toBe(true);
...
describe('leaderboard WebSocket (/leaderboard)', () => {
  it('rejects anonymous sockets', async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      setTimeout(() => reject(new Error('anonymous socket never errored')), 3000);
    });
    anon.close();
  });
