...
const SANDBOX_DIR = resolve(import.meta.dirname, '../../../infrastructure/sandbox');
...
describe.skipIf(!DOCKER_AVAILABLE)('sandbox isolation (docker-gated)', () => {
  ...
  afterAll(async () => {
    const container = realContainerId ? docker.getContainer(realContainerId) : null;
    if (container) {
  ...
  it('runs as non-root with no capabilities, no network, and a read-only rootfs', async () => {
    ...
    const stdout = execFileSync('docker', ['exec', realContainerId, 'sh', '-c', cmd], {
      encoding: 'utf8',
      timeout: 30_000,
    });
