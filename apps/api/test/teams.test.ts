...
async function register(
  ...
  return { token: body.tokens.accessToken, userId: body.user.id, username: body.user.username };
}
...
describe('teams API', () => {
  ...
  it('allows another user to join via the join code', async () => {
    ...
    expect(joined.members.some((m) => m.userId === b.userId && m.role === 'MEMBER')).toBe(true);
  });
  ...
  it('enforces one team per user', async () => {
    ...
    const teamTwo = await createTeam(teamTwoLeader.token, uniqueTeamName('epsilon'));
    const res = await api
      .post('/api/teams/join')
      .set('Authorization', `Bearer ${b.token}`)
  ...
  it('manages roles: promote, demote, and guard the last leader', async () => {
    ...
    const promoted = await api
      ...
      .expect(200);
    expect(
      (promoted.body.data as TeamDetailDto).members.find((m) => m.userId === b.userId)?.role,
    ).toBe('LEADER');
    ...
    const demoted = await api
      ...
      .expect(200);
    expect(
      (demoted.body.data as TeamDetailDto).members.find((m) => m.userId === a.userId)?.role,
    ).toBe('MEMBER');
  ...
  it('deletes the team as leader', async () => {
    ...
    await api.delete(`/api/teams/${team.id}`).set('Authorization', `Bearer ${a.token}`).expect(200);
  ...
  it('lists teams and supports search', async () => {
    ...
    expect((res.body.data as TeamDto[]).some((t) => t.id === team.id)).toBe(true);

    const search = await api.get(`/api/teams?search=${encodeURIComponent(name.slice(3, 12))}`).expect(200);
    expect((search.body.data as TeamDto[]).some((t) => t.id === team.id)).toBe(true);
  });
});