...
describe("evaluateUnlockRule", () => {
  ...
  it("unlocks ALWAYS rules", () => {
    expect(evaluateUnlockRule(rule({ type: "ALWAYS" }), base)).toEqual({
      locked: false,
    });
  });
  ...
  it("unlocks TIME rules with no unlock time", () => {
    expect(
      evaluateUnlockRule(rule({ type: "TIME", unlockAt: null }), base),
    ).toEqual({
      locked: false,
    });
  });
  ...
  it("treats PREREQUISITE with no requirements as unlocked", () => {
    expect(
      evaluateUnlockRule(
        rule({ type: "PREREQUISITE", prerequisiteChallengeIds: [] }),
        base,
      ),
  ...
  it("locks SCORE rules until the threshold is reached", () => {
    const locked = evaluateUnlockRule(
      rule({ type: "SCORE", minScore: 600 }),
      base,
    );
    expect(locked).toEqual({ locked: true, reason: "score" });

    const unlocked = evaluateUnlockRule(
      rule({ type: "SCORE", minScore: 500 }),
      base,
    );
    expect(unlocked).toEqual({ locked: false });
  });
  ...
  it("unlocks SCORE rules at the exact threshold", () => {
    expect(
      evaluateUnlockRule(rule({ type: "SCORE", minScore: 500 }), base),
    ).toEqual({
      locked: false,
    });
  });
  ...
  it("maps anonymity to zero score and an empty solved set", () => {
    ...
    expect(
      evaluateUnlockRule(
        rule({ type: "PREREQUISITE", prerequisiteChallengeIds: [1] }),
        anonymous,
      ),
    ).toEqual({ locked: true, reason: "prerequisite" });
    expect(
      evaluateUnlockRule(rule({ type: "SCORE", minScore: 100 }), anonymous),
    ).toEqual({
      locked: true,
      reason: "score",
    });
    expect(
      evaluateUnlockRule(rule({ type: "TIME", unlockAt: null }), anonymous),
    ).toEqual({
      locked: false,
    });
  });
