...
beforeAll(async () => {
  ...
  ch.push(
    await createPublishedChallenge(
      adminToken,
      `Event Gate 0 ${runSuffix}`,
      FLAG_0,
    ),
  );
  ch.push(
  ...
  const unlockPrereq = addEventChallenge(ch[1]!, {
    type: "PREREQUISITE",
    requireChallengeIds: [ch[0]!],
  });
  const unlockScore = addEventChallenge(ch[2]!, {
    type: "SCORE",
    minScore: 110,
  });
  const unlockFuture = addEventChallenge(ch[3]!, {
    type: "TIME",
    unlockAt: hourOffset(2 * 3_600_000),
  });
...
describe("event lifecycle", () => {
  it("lists events with time-derived status", async () => {
    ...
    expect(items.find((e) => e.id === eventScheduled)?.status).toBe(
      "SCHEDULED",
    );
  ...
  it("only allows registration while scheduled or running", async () => {
    await api
      .post(`/api/events/${eventDraft}/join`)
      .set("Authorization", `Bearer ${userBToken}`)
      .expect(400);
    await api
      .post(`/api/events/${eventScheduled}/join`)
      .set("Authorization", `Bearer ${userBToken}`)
      .expect(200);
    await api
      .post(`/api/events/${eventEnded}/join`)
      .set("Authorization", `Bearer ${userBToken}`)
      .expect(400);
    await api
      .post(`/api/events/${eventRunning}/join`)
      .set("Authorization", `Bearer ${userBToken}`)
      .expect(200);
  });
...
describe("event unlock gating", () => {
  it("locks everything for anonymous / non-registered viewers", async () => {
    const res = await api
      .get(`/api/events/${eventRunning}/challenges`)
      .expect(200);
  ...
  it("gates the challenge list via ?event=", async () => {
    ...
    expect(items).toHaveLength(4);
    expect(
      items.every((c) => c.locked && c.lockedReason === "join_required"),
    ).toBe(true);
  });
  ...
  it("treats drafts, scheduled and ended events as fully locked", async () => {
    const draftChallenges = await api
      .get(`/api/events/${eventDraft}/challenges`)
      .expect(200);
    const schedChallenges = await api
      .get(`/api/events/${eventScheduled}/challenges`)
      .expect(200);
    const endedChallenges = await api
      .get(`/api/events/${eventEnded}/challenges`)
      .expect(200);
  ...
  it("locks again after leaving the event", async () => {
    await api
      .post(`/api/events/${eventRunning}/leave`)
      .set("Authorization", `Bearer ${userBToken}`)
      .expect(200);
    const res = await api
      .get(`/api/events/${eventRunning}/challenges`)
    ...
    const items = res.body.data as EventChallengeDto[];
    expect(
      items.every((i) => i.locked && i.lockedReason === "join_required"),
    ).toBe(true);
  });
});
...
describe("admin event endpoints", () => {
  ...
  it("updates and removes event challenges", async () => {
    ...
    const updated = await api
      ...
      .expect(200);
    expect((updated.body.data as EventChallengeDto).unlockRule?.type).toBe(
      "ALWAYS",
    );
  ...
  it("updates and deletes events", async () => {
    const updated = await api
      ...
      .expect(200);
    expect((updated.body.data as EventSummaryDto).title).toContain(
      "Renamed Draft",
    );
