...
describe('POST /api/auth/register', () => {
  ...
  it('rejects an existing email (409)', async () => {
    const email = uniqueEmail();
    await api.post('/api/auth/register').send({ email, username: uniqueUsername('bob1'), password: 'password123' });
    const res = await api.post('/api/auth/register').send({
      email,
  ...
  it('rejects an existing username (409)', async () => {
    ...
    const res = await api.post('/api/auth/register').send({ email, username, password: 'password123' });
    expect(res.status).toBe(201);
    const res2 = await api.post('/api/auth/register').send({ email: uniqueEmail(), username, password: 'password123' });
...
describe('POST /api/auth/login', () => {
  ...
  beforeAll(async () => {
    await api.post('/api/auth/register').send({ email, username: uniqueUsername('dana'), password });
  });
...
describe('GET /api/auth/me', () => {
  ...
  beforeAll(async () => {
    const res = await api
      .post('/api/auth/register')
      .send({ email, username: uniqueUsername('erin'), password: 'password123' });
...
describe('refresh token rotation', () => {
  ...
  beforeAll(async () => {
    const res = await api
      .post('/api/auth/register')
      .send({ email, username: uniqueUsername('farid'), password: 'password123' });
...
describe('logout and logout-all', () => {
  ...
  beforeAll(async () => {
    const reg = await api
      .post('/api/auth/register')
      .send({ email, username: uniqueUsername('gina'), password: 'password123' });
...
describe('password reset', () => {
  ...
  it('issues a reset token (dev), resets the password, and revokes sessions', async () => {
    ...
    const oldLogin = await api.post('/api/auth/login').send({ email, password: originalPassword });
