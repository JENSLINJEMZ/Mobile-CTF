...
describe('scoring math', () => {
  ...
  it('solve result adds bonus only for the first solver', () => {
    expect(computeSolveResult(200, 0, true)).toEqual({ pointsAwarded: 220, firstBlood: true });
    expect(computeSolveResult(200, 0, false)).toEqual({ pointsAwarded: 200, firstBlood: false });
    expect(computeSolveResult(200, 50, true)).toEqual({ pointsAwarded: 170, firstBlood: true });
  });

  it('sumTotalScore aggregates awarded points', () => {
    expect(sumTotalScore([{ pointsAwarded: 220 }, { pointsAwarded: 75 }, { pointsAwarded: 40 }])).toBe(335);
