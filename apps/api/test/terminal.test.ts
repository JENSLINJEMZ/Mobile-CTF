...
async function connectAuthed(): Promise<Socket> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    setTimeout(
      () => reject(new Error("terminal socket never connected")),
      4000,
    );
  });
  return socket;
...
describe("terminal WebSocket (/terminal)", () => {
  ...
  it("rejects anonymous sockets", async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      anon.on("connect", () =>
        reject(new Error("anonymous terminal socket connected")),
      );
      setTimeout(
        () => reject(new Error("anonymous terminal socket never errored")),
        4000,
      );
    });
    anon.close();
  ...
  it("relays input to the container and output back to the room", async () => {
    ...
    const inputAck = await emitWithAck(socket, "terminal:input", {
      sessionId,
      data: "whoami\n",
    });
...
describe("expiry sweep", () => {
  it("expires and kills sessions past their TTL", async () => {
    ...
    const after = await prisma.terminalSession.findUnique({
      where: { id: session.id },
    });
