...
export async function createTeam(body: CreateTeamRequest): Promise<TeamDetailDto> {
...
export async function updateMemberRole(
  ...
  return api.patch<TeamDetailDto>(`/teams/${teamId}/members/${userId}/role`, { role }, { auth: true });
}

export async function removeMember(teamId: number, userId: number): Promise<TeamDetailDto> {
  return api.del<TeamDetailDto>(`/teams/${teamId}/members/${userId}`, { auth: true });
}
