...
function SectionChips({ active, onChange }: { active: Section; onChange: (section: Section) => void }) {
  return (
    <ThemedView style={styles.chips}>
      {SECTIONS.map((value) => {
        ...
        return (
          ...
          >
            <ThemedText type="smallBold" style={{ color: isActive ? '#ffffff' : undefined }}>
              {value}
            </ThemedText>
          </Pressable>
        );
    ...
    <ThemedView style={styles.modeRow}>
      {options.map((option) => {
        ...
        return (
          ...
          >
            <ThemedText type="small" style={{ color: isActive ? '#ffffff' : undefined }}>
              {option.label}
            </ThemedText>
          </Pressable>
        );
  ...
  const run = () => {
    try {
      ...
      if (mode === 'rot13') {
        ...
        if (mode === 'hex') {
          setOutput(Array.from(utf8ToBytes(input), (b) => b.toString(16).padStart(2, '0')).join(' '));
        }
  ...
  return (
    <ToolCard title="Encoding">
      ...
      <TextArea
        ...
        placeholder={mode === 'hex' && direction === 'decode' ? 'Paste hex bytes…' : 'Paste text…'}
        multiline
      />
      <Pressable onPress={run} style={({ pressed }) => [styles.actionButton, pressed && styles.pressed]}>
...
function CipherTool() {
  ...
  return (
    <ToolCard title="Ciphers">
      ...
      {cipher !== 'frequency' ? <OutputBlock value={result} error={error ?? undefined} /> : null}
        ...
        <ThemedView style={styles.freqBox}>
          ...
          {frequency.topTrigrams.length > 0 ? (
            <ThemedView style={styles.trigramBox}>
              ...
              <ThemedText type="small" themeColor="textSecondary">
                {frequency.topTrigrams.map((entry) => `${entry.trigram} (${entry.count})`).join(' · ')}
              </ThemedText>
            </ThemedView>
          ) : null}
        </ThemedView>
...
function HashIdentifyTool() {
  ...
  return (
    <ToolCard title="Hash identifier">
      ...
      <TextArea value={input} onChangeText={setInput} placeholder="Paste a hash…" multiline />
      {result.candidates.length === 0 && input.trim() ? (
        <ThemedText style={{ color: ERROR_COLOR }}>No known hash format matches.</ThemedText>
      ) : null}
      {result.candidates.map((candidate) => (
        <ThemedView key={candidate.name} style={styles.hashRow}>
          <ThemedText type="smallBold">{candidate.name}</ThemedText>
          <ThemedText type="small" themeColor="textSecondary">
            {candidate.hexLength != null ? `${candidate.hexLength} hex chars` : 'prefixed format'} ·{' '}
...
function JwtDecodeTool() {
  ...
  return (
    <ToolCard title="JWT decoder">
      ...
      <TextArea value={input} onChangeText={setInput} placeholder="Paste a JWT (header.payload.signature)…" multiline />
      {hasInput && !decoded.validStructure ? (
        <ThemedText style={{ color: ERROR_COLOR }}>{decoded.errors.join(' ')}</ThemedText>
      ) : null}
      {hasInput && decoded.validStructure ? (
        <ThemedView style={styles.jwtBlock}>
          ...
          {decoded.payload?.exp != null ? (
            <ThemedText type="small" themeColor="textSecondary">
              Expires: {new Date((decoded.payload.exp as number) * 1000).toString()}
...
function FileAnalyzeTool() {
  ...
  const analysis = useMemo(() => {
    ...
    try {
      ...
      if (exif) {
        ...
        if (exif.orientation) exifLines.push(`Orientation: ${exif.orientation}`);
        if (gps) exifLines.push(gps);
      }
      const body = exifLines.length > 0 ? `\nEXIF:\n${exifLines.join('\n')}` : '';
  ...
  return (
    <ToolCard title="Hex / file analysis">
      <ThemedText type="small" themeColor="textSecondary">
        Paste raw hex bytes (from `xxd`, `od -tx1`, or a file dump) to inspect magic bytes, size, EXIF and
        offsets.
      </ThemedText>
      <TextArea value={input} onChangeText={setInput} placeholder="e.g. ff d8 ff e0 …" multiline />
      {analysis?.error ? <ThemedText style={{ color: ERROR_COLOR }}>{analysis.error}</ThemedText> : null}
      {analysis?.dump ? <OutputBlock value={analysis.dump} error={undefined} /> : null}
    </ToolCard>
  );
}
