...
| Root fs       | `--read-only` rootfs; only `/tmp` and `/home/ctf` are re-mounted as writable, noexec `tmpfs` (`size=8m`, `noexec`+`nosuid`+`nodev`). `/home/ctf` tmpfs is mounted `uid=10001,gid=10001` so the `ctf` user owns its own home. |
